...
namespace ic {
  ...
  SimpleTCPStream::SimpleTCPStream(TCPSocket &server, size_t size)
    : TCPSocket(accept(server.getSocket(), NULL, NULL)) {
      tpport_t port;
  ...
  SimpleTCPStream::SimpleTCPStream(const IPV4Host &host, tpport_t port, size_t size)
    : TCPSocket(AF_INET, SOCK_STREAM, IPPROTO_TCP) {
  ...
  SimpleTCPStream::SimpleTCPStream()
    : TCPSocket(PF_INET, SOCK_STREAM, IPPROTO_TCP) {
      // Do nothing extra.
  }
  ...
  ssize_t SimpleTCPStream::read(char *bytes, size_t length, timeout_t timeout)
  {
    ...
    while (totalrecv < length) {
      ...
      //cout << "received " << rlen << " bytes, remaining " << length - totalrecv << std::flush;
  ...
  ssize_t SimpleTCPStream::readAll(char *bytes, size_t length, timeout_t timeout)
  {
    // Declare local variables.
    ssize_t rlen = 0;
    size_t totalrecv = 0;
    ...
    while (totalrecv < length) {
      ...
      //cout << "receiveAll " << rlen << " bytes, remaining " << length - totalrecv << std::flush;
  ...
  {
    return TCPSocket::isPending(pending, timeout);
  }
